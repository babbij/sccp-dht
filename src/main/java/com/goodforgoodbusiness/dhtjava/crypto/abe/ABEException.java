...
public class ABEException extends Exception {
	...
	public static ABEException forResult(int code) {
		ErrorResult.DECRYPTION_FAILED.toString();
		
		ErrorResult result = ErrorResult.get(code);
		if (result != null) {
			return new ABEException("ABE returned error: " + result.name() + " [" + code + "]");
		}
		else {
			return new ABEException("ABE returned unknown error [" + code + "]");
		}
	}
	
	private ABEException(String message) {
		super(message);
	}
}
