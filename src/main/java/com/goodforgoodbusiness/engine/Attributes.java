...
public final class Attributes {
	...
	public static String forShare(KPABEPublicKey key, TriTuple tuple, Optional<ZonedDateTime> start, Optional<ZonedDateTime> end) {
		...
		if (start.isPresent()) {
			pattern += start.map(Attributes::toTimeRepresentation).map(epochsec -> " AND time >= " + epochsec).get();
		}
		
		if (end.isPresent()) {
			pattern += end.map(Attributes::toTimeRepresentation).map(epochsec -> " AND time < " + epochsec).get();
		}
